...
typedef struct {
    ...
    int threads, batch, n_tokens, warmup, iters, seed, top_k, logits_limit, no_print, max_ctx;
    float temp;
    double load_time_ms;        /* model load (cold-start component) */
} Args;
...
static void usage(const char *prog)
{
    fprintf(stderr,
        ...
        "  --precision fp16|bf16|int8|auto (default auto; derived from .ecm weight dtype)\n"
        "  --threads N            (default 1)\n"
        "  --batch N              (default 1)\n"
        ...
        "  --max-ctx N            max context length (default 0 = use model's n_ctx)\n"
                            ...
                            int *out_ids, int *out_n, double *ttft_ms)
{
    ...
    /* reset KV: zero caches (sized by n_kv_embd; == n_embd for GPT-2, < for GQA) */
    int eff_ctx = gpt2_eff_ctx(g);
    size_t kv = (size_t)g->n_layer * (size_t)g->batch_cap * (size_t)eff_ctx * (size_t)g->n_kv_embd;
...
static int mode_generate(GPT2 *g, const Args *a, const Affinity *aff)
{
    ...
    fprintf(o, "  \"threads\": %d,\n", a->threads);
    fprintf(o, "  \"batch\": %d,\n", a->batch);
    fprintf(o, "  \"load_time_ms\": %.3f,\n", a->load_time_ms);
    emit_affinity(o, aff);
    fprintf(o, ",\n  \"n_prompt_tokens\": %d,\n", n);
    fprintf(o, "  \"n_generated_tokens\": %d,\n", a->n_tokens);
    fprintf(o, "  \"ttft_ms\": %.3f,\n", ttft);
    fprintf(o, "  \"cold_start_ms\": %.3f,\n", a->load_time_ms + ttft);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg_is(argv[i], "--max-ctx")) a.max_ctx = atoi(arg_get(argc, argv, &i, "--max-ctx"));
    ...
    else if (!strcmp(a.precision, "bf16")) want_precision = 2;
    else if (!strcmp(a.precision, "int8")) want_precision = 0;
    else if (strcmp(a.precision, "auto")) {
        fprintf(stderr, "unknown precision: %s (use fp16|bf16|int8|auto)\n", a.precision);
        return 2;
    }

    GPT2 g;
    double t_load0 = now_sec();
    if (gpt2_load(&g, a.model, a.batch, a.max_ctx) != 0) {
        fprintf(stderr, "failed to load model\n");
        return 1;
    }
    a.load_time_ms = (now_sec() - t_load0) * 1000.0;
    if (want_precision >= 0 && want_precision != g.precision) {
        const char *have = g.precision == 2 ? "bf16" : (g.precision ? "fp16" : "int8");
        fprintf(stderr, "model file uses %s weights, but --precision %s was requested\n",
                have, a.precision);
        gpt2_free(&g);
        return 2;
    }
    a.precision = g.precision == 2 ? "bf16" : (g.precision ? "fp16" : "int8");
